<?php
	...
	class CBXWPBookmark_Admin {
		...
		public function get_settings_fields() {
			...
			$table_html = '<p><a id="cbxwpbookmark_info_trig" href="#">' . esc_html__( 'Show/hide details', 'cbxwpbookmark' ) . '</a></p>';
			...
			if ( $mybookmark_pageid > 0 ) {
				$mybookmark_customizer_url      = add_query_arg( array( 'autofocus' => array( 'panel' => 'cbxwpbookmark' ), 'url' => cbxwpbookmarks_mybookmark_page_url() ), admin_url( 'customize.php' ) );
...
<a class="button button-primary" onclick="return confirm(\'%s\')" href="%s">Reset Data</a>',
		...
		public function delete_bookmark( $object_id ) {
			...
			if ( is_array( $bookmarks ) && sizeof( $bookmarks ) > 0 ) {
				foreach ( $bookmarks as $bookmark ) {
					...
					if(!in_array($object_type, $object_types)) return ;
		...
		public function enqueue_styles( $hook ) {
			...
			if ( $page == 'cbxwpbookmark_settings' || $page == 'cbxwpbookmark' || $page == 'cbxwpbookmarkcats' ) {
		...
		public function enqueue_scripts( $hook ) {
			...
			if ( $page == 'cbxwpbookmark_settings' ) {
				...
				//wp_register_script( 'cbxbookmarkchoosen', plugin_dir_url( __FILE__ ) . '../assets/js/chosen.jquery.min.js', array( 'jquery' ), $this->version, true );
				...
				$cbxwpbookmark_admin_js_vars = apply_filters( 'cbxwpbookmark_admin_js_vars',
					array(
				...
				wp_localize_script( 'cbxwpbookmark-admin', 'cbxwpbookmark_admin', $cbxwpbookmark_admin_js_vars );
		...
		public function admin_pages() {
			...
			//review listing page
			$bookmark_list_page_hook = add_menu_page( esc_html__( 'CBX WP Bookmark Listing', 'cbxwpbookmark' ),
			...
			$bookmark_category_page_hook = add_submenu_page(
				'cbxwpbookmark',
				esc_html__( 'CBX WP Bookmark Category Listing', 'cbxwpbookmark' ),
			...
			if ( isset( $submenu['cbxwpbookmark'][0][0] ) ) {
				$submenu['cbxwpbookmark'][0][0] = esc_html__( 'User Bookmarks', 'cbxwpbookmark' );
			}
		...
		public function display_plugin_admin_settings() {
			echo cbxwpbookmark_get_template_html( 'admin/setting_display.php', array( 'ref' => $this, 'settings_api' => $this->settings_api ) );
		...
		public function plugin_activate_upgrade_notices() {
			...
			if ( get_transient( 'cbxwpbookmark_activated_notice' ) ) {
				echo '<div style="border-left-color: #9c27b0;" class="notice notice-success is-dismissible">';
				...
				echo '<p>' . sprintf( __( 'Check <a style="color:#9c27b0 !important; font-weight: bold;" href="%s">Plugin Setting</a> | <a href="%s" target="_blank"><span class="dashicons dashicons-external"></span> Documentation</a>', 'cbxwpbookmark' ), admin_url( 'admin.php?page=cbxwpbookmark_settings' ), 'https://codeboxr.com/product/cbx-wordpress-bookmark/' ) . '</p>';
				echo '</div>';
		...
		public function pro_addon_compatibility_campaign() {
			...
			if ( in_array( 'cbxwpbookmarkaddon/cbxwpbookmarkaddon.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || defined( 'CBXWPBOOKMARKADDON_PLUGIN_NAME' ) ) {
				...
				if(version_compare($pro_plugin_version,'1.1.10', '<') ){
					echo '<div style="border-left-color: #9c27b0;" class="notice notice-success is-dismissible"><p>' . sprintf(esc_html__( 'CBX Bookmark Pro Addon V%s or any previous version is not 100%% compatible with CBX Bookmark Core V1.5.3 or later. Please update CBX Bookmark Pro Addon to version 1.1.10 or latest. - Codeboxr Team', 'cbxmcratingreview' ), $pro_plugin_version) . '</p></div>';
				}


			} else {
				echo '<div style="border-left-color: #9c27b0;" class="notice notice-success is-dismissible"><p>' . sprintf( __( '<a target="_blank" href="%s">CBX Bookmark Pro Addon</a> has extended features, settings, widgets and shortcodes. try it  - Codeboxr Team', 'cbxwpbookmark' ), 'https://codeboxr.com/product/cbx-wordpress-bookmark/' ) . '</p></div>';
			}
			...
			if ( in_array( 'cbxwpbookmarkmycred/cbxwpbookmarkmycred.php.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || defined( 'CBXWPBOOKMARKMYCRED_PLUGIN_NAME' ) ) {
				...
				echo '<div style="border-left-color: #9c27b0;" class="notice notice-success is-dismissible"><p>' . sprintf( __( '<a target="_blank" href="%s">CBX Bookmark myCred Addon</a> has myCred integration. try it  - Codeboxr Team', 'cbxwpbookmark' ), 'https://codeboxr.com/product/cbx-bookmark-mycred-addon/' ) . '</p></div>';
			}
				...
				array(
					...
					'attributes'      => apply_filters( 'cbxwpbookmark_mycat_block_attributes',
						array(
							'title'          => array(
								'type'    => 'string',
								'default' => esc_html__( 'My Bookmark Categories', 'cbxwpbookmark' ),
							),
							'order'          => array(
								'type'    => 'string',
								'default' => 'ASC',
		...
		public function plugin_action_links( $links ) {
			$action_links = array(
				'settings' => '<a style="color:#9c27b0 !important; font-weight: bold;" href="' . admin_url( 'admin.php?page=cbxwpbookmark_settings' ) . '" aria-label="' . esc_attr__( 'View settings', 'cbxwpbookmark' ) . '">' . esc_html__( 'Settings', 'cbxwpbookmark' ) . '</a>',
			);
		...
		public function plugin_row_meta( $links_array, $plugin_file_name, $plugin_data, $status ) {
			if ( strpos( $plugin_file_name, CBXWPBOOKMARK_BASE_NAME ) !== false ) {
				...
				$links_array[] = '<a target="_blank" style="color:#9c27b0 !important; font-weight: bold;" href="https://wordpress.org/support/plugin/cbxwpbookmark/" aria-label="' . esc_attr__( 'Free Support', 'cbxwpbookmark' ) . '">' . esc_html__( 'Free Support', 'cbxwpbookmark' ) . '</a>';

				$links_array[] = '<a target="_blank" style="color:#9c27b0 !important; font-weight: bold;" href="https://wordpress.org/plugins/cbxwpbookmark/#reviews" aria-label="' . esc_attr__( 'Reviews', 'cbxwpbookmark' ) . '">' . esc_html__( 'Reviews', 'cbxwpbookmark' ) . '</a>';

				$links_array[] = '<a target="_blank" style="color:#9c27b0 !important; font-weight: bold;" href="https://codeboxr.com/documentation-for-cbx-bookmark-for-wordpress/" aria-label="' . esc_attr__( 'Documentation', 'cbxwpbookmark' ) . '">' . esc_html__( 'Documentation', 'cbxwpbookmark' ) . '</a>';
				...
				if ( in_array( 'cbxwpbookmarkaddon/cbxwpbookmarkaddon.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || defined( 'CBXWPBOOKMARKADDON_PLUGIN_NAME' ) ) {
					...
					$links_array[] = '<a target="_blank" style="color:#9c27b0 !important; font-weight: bold;" href="https://codeboxr.com/product/cbx-wordpress-bookmark/" aria-label="' . esc_attr__( 'Try Pro Addon', 'cbxwpbookmark' ) . '">' . esc_html__( 'Try Pro Addon', 'cbxwpbookmark' ) . '</a>';
				}
        ...
        public function plugin_update_message_pro_addons(){
                echo ' '.sprintf(__('Check how to <a style="color:#9c27b0 !important; font-weight: bold;" href="%s"><strong>Update manually</strong></a> , download latest version from <a style="color:#9c27b0 !important; font-weight: bold;" href="%s"><strong>My Account</strong></a> section of Codeboxr.com', 'cbxwpbookmark'), 'https://codeboxr.com/manual-update-pro-addon/', 'https://codeboxr.com/my-account/');
