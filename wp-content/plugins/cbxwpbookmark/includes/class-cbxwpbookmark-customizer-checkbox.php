	...
	class CBXWPBookmark_Customizer_Control_Checkbox extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<div class="cbxwpbookmark_checkbox_control">
				...
				<?php } ?>
				<input type="hidden" id="<?php echo esc_attr( $this->id ); ?>" name="<?php echo esc_attr( $this->id ); ?>" value="<?php echo esc_attr( $this->value() ); ?>" class="customize-control-sortable-cbxwpbookmark-checkbox" <?php $this->link(); ?> />
				<div class="sortable_pills<?php echo ( $this->sortable ? ' sortable' : '' ) . ( $this->fullwidth ? ' fullwidth_pills' : '' ); ?>">
					<?php foreach ( $reordered_choices as $key => $value ) { ?>
						<label class="checkbox-label"> <input type="checkbox" name="<?php echo esc_attr( $key ); ?>" value="<?php echo esc_attr( $key ); ?>" <?php checked( in_array( esc_attr( $key ), $saved_choices, true ), true ); ?> class="sortable-cbxwpbookmark-checkbox" /> <span class="sortable-pill-title"><?php echo esc_attr( $value ); ?></span>
