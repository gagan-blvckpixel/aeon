	...
	class CBXWPBookmark_Customizer_Control_Select2 extends WP_Customize_Control {
		...
		public function render_content() {
			...
			<div class="cbxwpbookmark_dropdown_select2_control">
				...
				<?php } ?>
				<input type="hidden" id="<?php echo esc_attr( $this->id ); ?>" class="cbxwpbookmark-customize-control-dropdown-select2" value="<?php echo esc_attr( $this->value() ); ?>" name="<?php echo esc_attr( $this->id ); ?>" <?php $this->link(); ?> /> <select name="select2-list-<?php echo( $this->multiselect ? 'multi[]' : 'single' ); ?>" class="cbxwpbookmark-customize-control-select2" data-placeholder="<?php echo $this->placeholder; ?>" <?php echo( $this->multiselect ? 'multiple="multiple" ' : '' ); ?>>
					<?php
