<?php
	...
	class CBXWPBookmark_Public {
		...
		public function bookmarked_posts_shortcode( $attr ) {
			...
			if($title != ''){
				$title = '<h3 class="cbxwpbookmark-title cbxwpbookmark-title-postlist">'.$title.'</h3>';
			}

			return '<div class="cbxwpbookmark-mylist-wrap">' . $title . '<ul class="cbxwpbookmark-mylist cbxwpbookmark-mylist-sc ' . $extra_css_class . '" >' . cbxbookmark_post_html( $attr ) . '</ul>' . $show_loadmore_html . '</div>';
		...
		public function enqueue_scripts() {
			...
			wp_register_script( 'cbxwpbookmarkpublicjs', plugin_dir_url( __FILE__ ) . '../assets/js/cbxwpbookmark-public.js', array( 'cbxwpbookmark-events', 'jquery'), CBXWPBOOKMARK_PLUGIN_VERSION, true );
